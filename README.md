...
Using the [post type API](https://github.com/hlashbrooke/WordPress-Plugin-Template/blob/master/includes/lib/class-simple-system-status-post-type.php) and the wrapper function from the main plugin class you can easily register new post types with one line of code. For exapmle if you wanted to register a `listing` post type then you could do it like this:

`Simple_System_Status()->register_post_type( 'listing', __( 'Listings', 'simple-system-status' ), __( 'Listing', 'simple-system-status' ) );`

*Note that the `Simple_System_Status()` function name and the `simple-system-status` text domain will each be unique to your plugin after you have used the cloning script.*
...
Using the [taxonomy API](https://github.com/hlashbrooke/WordPress-Plugin-Template/blob/master/includes/lib/class-simple-system-status-taxonomy.php) and the wrapper function from the main plugin class you can easily register new taxonomies with one line of code. For exapmle if you wanted to register a `location` taxonomy that applies to the `listing` post type then you could do it like this:

`Simple_System_Status()->register_taxonomy( 'location', __( 'Locations', 'simple-system-status' ), __( 'Location', 'simple-system-status' ), 'listing' );`
